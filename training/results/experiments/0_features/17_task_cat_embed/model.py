...
class HeteroGNN(torch.nn.Module):
    def __init__(self, hidden_channels: int, num_mpn_layers: int): 
        ...
        self.task_embedding     = nn.Embedding(1, projection_size - 2)
    ...
    def forward(self, data):
        ...
        num_task_in_batch   = x_dict['task'].size(0) 
        task_embedding      = self.task_embedding.weight.repeat(num_task_in_batch, 1)    
        x_dict['task']      = torch.cat([x_dict['task'], task_embedding], dim=1)
