...
class HeteroGNN(torch.nn.Module):
    ...
    def _get_hetero_conv(self, in_channels, out_channels): 
        conv = HeteroConv({
            ...
            ("task", "mapped_to", "pe"):        GraphConv(in_channels, out_channels), 
            ("pe", "rev_mapped_to", "task"):    GraphConv(in_channels, out_channels), 
            ("router", "link", "router"):       GraphConv(in_channels, out_channels), 
            ("router", "interface", "pe"):      GraphConv(in_channels, out_channels), 
            ("pe", "rev_interface", "router"):  GraphConv(in_channels, out_channels),
        }, aggr="sum")
